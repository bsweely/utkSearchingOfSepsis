...
%   Copyright 2008-2017 The MathWorks, Inc.
    ...
    methods
        ...
        %   plotPartialDependence(MODEL,VAR) takes a fitted Regression TreeBagger
        ...
        %
        %   Examples:
        %      % Partial Dependence Plot of Regression TreeBagger
        %      load carsmall
        %      tbl = table(Weight,Cylinders,Origin,MPG);
        %      t = TreeBagger(10,tbl,'MPG','Method','regression');
        %
        %      plotPartialDependence(t,'Weight');
        %      plotPartialDependence(t,{'Weight','Origin'});
        %      plotPartialDependence(t,[1,3]);
        %
        %      % Obtain optional output Axes handle
        %      ax = plotPartialDependence(t,1);
        %
        %      % With additional Data
        %      load carbig
        %      tbl2 = table(Weight,Cylinders,Origin);
        %      plotPartialDependence(t,'Weight',tbl2);
        %      plotPartialDependence(t,1,tbl2);
        %
        %      % With optional name-value pairs
        %      plotPartialDependence(t,1,tbl2,'NumObservationsToSample',100);
        %      plotPartialDependence(t,1,tbl2,'UseParallel',true);
        %      plotPartialDependence(t,1,tbl2,'UseParallel',true,'Conditional','none');
        %      
        %      % Plot the Individual Conditional Expectation
        %      plotPartialDependence(t,1,tbl2,'Conditional','absolute');
        %
        %      % Provide alternative query points
        %      xi = linspace(min(Weight),max(Weight))';
        %      plotPartialDependence(t,1,'QueryPoints',xi);
        %      
        %      xi = cell(1,2);
        %      xi{1} = linspace(min(Weight),max(Weight))';
        %      xi{2} = linspace(min(Cylinders),max(Cylinders))';
        %      plotPartialDependence(t,[1,2],'QueryPoints',xi);
